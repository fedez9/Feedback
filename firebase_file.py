...
def save_user_data(chat_id: int, user_id: int, user_data: dict) -> None:
    """
    Salva i dati di un SINGOLO utente sul nodo 'group_users/{chat_id}/{user_id}',
    senza riscrivere l'intero albero (a differenza di save_group_users).
    """
    try:
        ref = db.reference(f'group_users/{chat_id}/{user_id}')
        ref.set(user_data)
    except Exception as e:
        logger.error(f"Errore save_user_data su Firebase per l'utente {user_id}: {e}")
...
def save_user_stats(user_id: int, user_stats: dict) -> None:
    """
    Salva le statistiche di un SINGOLO utente sul nodo 'stats/{user_id}',
    senza riscrivere l'intero albero (a differenza di save_stats).
    """
    try:
        ref = db.reference(f'stats/{user_id}')
        ref.set(user_stats)
    except Exception as e:
        logger.error(f"Errore save_user_stats su Firebase per l'utente {user_id}: {e}")
