...
from firebase_file import save_user_stats, load_stats, load_group_users
...
def update_feedback_stats(stats: Dict[int, dict], sender_id: int, sender_username: str, target_id: int, target_username: str) -> None:
    ...
    # 4) Salva su Firebase SOLO i due utenti coinvolti (mittente e destinatario),
    # invece di riscrivere l'intero albero 'stats' con tutti gli utenti.
    # NOTA: il precedente ricalcolo di "proporzione" per ogni utente ad ogni
    # feedback è stato rimosso perché quel campo non risulta letto/usato
    # da nessun'altra parte del codice (era solo un costo, mai un beneficio).
    save_user_stats(sender_id, sender_stats)
    save_user_stats(target_id, target_stats)
