...
from firebase_file import load_stats, save_user_data, load_group_users, load_user_data
...
from utils import add_auth, remove_auth, list_admins, list_verified_users, list_feedback_received, list_feedback_sent, handle_pagination_callback, load_admin_ids, list_limited_users, list_users_to_limit, list_users_to_warn
...
async def traccia_utente(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Traccia se qualcosa è realmente cambiato, per evitare scritture inutili su Firebase
    data_changed = False
    ...
    # 4. Salva su Firebase SOLO se qualcosa è effettivamente cambiato, e SOLO
    # il singolo utente toccato (non l'intero dizionario group_users).
    if data_changed:
        save_user_data(chat_id, user.id, current_user_data)
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "confirm":
        ...
        # Salva solo mittente e destinatario, non l'intero database group_users
        save_user_data(origin_chat, pending["user_id"], sender)
        save_user_data(origin_chat, pending["target_user_id"], target)
        # update_feedback_stats ora salva internamente solo i 2 utenti coinvolti
        # su Firebase (in precedenza qui veniva anche riscritto l'intero nodo
        # 'stats' una seconda volta, ridondante).
...
async def main() -> None:
    ...
    application.add_handler(CommandHandler("daavvisare", list_users_to_warn))
    ...
    application.add_handler(CallbackQueryHandler(handle_pagination_callback, pattern=r"^(pagina_verificati|pagina_ricevuti|pagina_inviati|pagina_limitati|pagina_dalimitare|pagina_daavvisare|pagina_admin)_(\d+)$"))
